...
{** Hash Library
    ...
    Changelog:
      v??? (20021002, Michael Buschbeck)
        * FFindKey checks hash value before comparing strings.
        * FFindKey caches one set of return values.
        * Create supports optional initial bucket count parameter.
  ...
  type
    ...
    THash = class
      protected
        ...
        {** Cache for FFindKey return values. }
        f_FindKeyCache_Hash: Cardinal;
        f_FindKeyCache_Key: string;
        f_FindKeyCache_k: Integer;
        f_FindKeyCache_x: Integer;
      ...
      public
        {** Our own constructor. }
        constructor Create(InitialItemCount: Cardinal = 0); reintroduce; virtual;
...
constructor THash.Create(InitialItemCount: Cardinal);
begin
  inherited Create;
  ...
  if InitialItemCount = 0 then begin
    self.f_CurrentItemShift := c_HashInitialItemShift;
  end else begin
    while InitialItemCount <> 0 do begin
      inc(self.f_CurrentItemShift);
      InitialItemCount := InitialItemCount shr 1;
    end;
    if self.f_CurrentItemShift < c_HashInitialItemShift then
      self.f_CurrentItemShift := c_HashInitialItemShift;
  end;
...
begin
  ...
  if (h = f_FindKeyCache_Hash) and (Key = f_FindKeyCache_Key) then begin
    k := f_FindKeyCache_k;
    x := f_FindKeyCache_x;
    result := true;
    exit;
  ...
  for i := 0 to High(self.f_Keys[k]) do
    if (self.f_Keys[k][i].Hash = h) {or true} then
      if (self.f_Keys[k][i].Key = Key) then begin
        ...
        result := true;
        x := i;
        f_FindKeyCache_Hash := h;
        f_FindKeyCache_Key := Key;
        f_FindKeyCache_k := k;
        f_FindKeyCache_x := x;
        break;
      end;
